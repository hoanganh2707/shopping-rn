...
export const DraggableChatbot = () => {
  ...
  const pan = useRef(new Animated.ValueXY({ x: SCREEN_WIDTH - FAB_SIZE - 16, y: SCREEN_HEIGHT - FAB_SIZE - 120 })).current;
  ...
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (e, gestureState) => {
        // Trigger pan handling only if movement is beyond a certain threshold (e.g. 5 pixels)
      ...
      onPanResponderRelease: (e, gestureState) => {
        ...
        // Y boundaries
        if (newY < 50) newY = 50;
        if (newY > SCREEN_HEIGHT - FAB_SIZE - 50) newY = SCREEN_HEIGHT - FAB_SIZE - 50;
